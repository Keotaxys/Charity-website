...
export default function HomeVideos() {
  ...
  const title = data ? (locale === 'lo' ? data.featured_title_lo : data.featured_title_en) : '';
    ...
    <section className="bg-gray-900 py-24 px-6 text-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          <div>
            <h2 className="text-4xl md:text-5xl font-black tracking-tight mb-4">{locale === 'lo' ? 'ວິດີໂອລ່າສຸດ' : 'LATEST VIDEOS'}</h2>
            <p className="text-gray-400 text-lg max-w-2xl">{locale === 'lo' ? 'ຕິດຕາມການລົງພື້ນທີ່ ແລະ ເບິ່ງຮອຍຍິ້ມທີ່ເກີດຂຶ້ນຈາກການຊ່ວຍເຫຼືອຂອງທ່ານ.' : 'Watch our recent missions and see the impact of your donations.'}</p>
          </div>
        ...
        <a href={data?.featured_url || `/${locale}/videos`} target="_blank" rel="noopener noreferrer" className="relative block w-full aspect-video bg-gray-800 rounded-3xl overflow-hidden shadow-2xl group cursor-pointer border border-gray-700">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-20 h-20 bg-teal-600 rounded-full flex items-center justify-center pl-2 shadow-lg group-hover:bg-pink-500 transition-colors duration-300"><svg className="w-10 h-10 text-white" fill="currentColor" viewBox="0 0 20 20"><path d="M4 4l12 6-12 6z" /></svg></div>
          </div>
