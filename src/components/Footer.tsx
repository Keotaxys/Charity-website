      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-center gap-8 mb-10">
          ...
          <div className="flex flex-wrap justify-center lg:justify-start items-center gap-3 md:gap-5 text-sm font-bold text-gray-500 uppercase tracking-wider">
            <Link href={`/${locale}/privacy`} className="hover:text-teal-600 transition-colors">
              {locale === 'lo' ? 'ນະໂຍບາຍຄວາມເປັນສ່ວນຕົວ' : 'PRIVACY POLICY'}
            </Link>
            <span className="text-pink-300">|</span>
            <Link href={`/${locale}/terms`} className="hover:text-teal-600 transition-colors">
              {locale === 'lo' ? 'ເງື່ອນໄຂການບໍລິຈາກ' : 'DISCLOSURE STATEMENT'}
            </Link>
            <span className="text-pink-300">|</span>
            <Link href={`/${locale}/about/contact`} className="hover:text-teal-600 transition-colors">
              {locale === 'lo' ? 'ຕິດຕໍ່ພວກເຮົາ' : 'CONTACT'}
            </Link>
            <span className="text-pink-300">|</span>
            <Link href={`/${locale}/financials`} className="hover:text-teal-600 transition-colors">
              {locale === 'lo' ? 'ລາຍງານການເງິນ' : 'FINANCIALS'}
            </Link>
          </div>
